...
const btnVoltarFlutuante = document.getElementById("btn-voltar-flutuante");
...
function tocarMusica(id) {
    ...
    btnVoltarFlutuante.classList.add("d-none");
...
btnVoltarFlutuante.addEventListener("click", function() {
    voltarParaPlaylistDoPlayer();
});

function mostrarControlesFlutuantes() {
    btnVoltarFlutuante.classList.remove("d-none");
    ...
    const musicasParaSalvar = musicas.map(function(musica) {
        return {
            ...
            letras: musica.letras ?
                musica.letras.map(function(letra) {
                    return {
                        id: letra.id,
                        nome: letra.nome,
                        inicio: letra.inicio,
...
function verificarArquivosDesconectados() {
    const temDesconectado = musicas.some(function(musica) {
        const audioDesconectado = !musica.arquivo || !(musica.arquivo instanceof File);

        const temLetraDesconectada = musica.letras && musica.letras.some(function(letra) {
            return !letra.arquivo || !(letra.arquivo instanceof File);
        });

        return audioDesconectado || temLetraDesconectada;
    });

    if (temDesconectado) {
        btnEscolherPasta.classList.remove("d-none");
    } else {
        btnEscolherPasta.classList.add("d-none");
    }
}


btnEscolherPasta.addEventListener("click", async function() {
    if (!window.showDirectoryPicker) {
        alert("Seu navegador não suporta escolha de pasta. Teste no Chrome ou Edge.");
        return;
    }
    ...
    try {
        ...
        listaArquivosPasta.innerHTML = "<p>Lendo arquivos da pasta...</p>";
        ...
        if (arquivosEncontrados.length === 0) {
            listaArquivosPasta.innerHTML =
                '<p class="text-warning">Nenhum arquivo encontrado nessa pasta.</p>';
            return;
        }

        mostrarMensagemPasta("✅ Arquivos encontrados e reconectados.", "success");

        await reconectarMusicasDaPasta(arquivosEncontrados);

    } catch (erro) {
        console.log(erro);
        mostrarMensagemPasta("⚠ Não foi possível escolher a pasta. Tente novamente.", "warning");
    }
});

async function reconectarMusicasDaPasta(arquivosEncontrados) {
    for (const musica of musicas) {

        // RECONECTAR ÁUDIO
        if (!musica.arquivo) {
            const audioEncontrado = arquivosEncontrados.find(function(arquivo) {
                return arquivo.nome.trim().toLowerCase() === (musica.nomeArquivoAudio || "").trim().toLowerCase();
            });
        ...
        if (musica.letras && musica.letras.length > 0) {
            ...
            for (const letra of musica.letras) {
                ...
                const letraEncontrada = arquivosEncontrados.find(function(arquivo) {
                    return arquivo.nome.trim().toLowerCase() === (letra.nome || "").trim().toLowerCase();
                });
    ...
    const aindaTemDesconectado = musicas.some(function(musica) {
        const audioDesconectado = !musica.arquivo;
...
function mostrarMensagemPasta(mensagem, tipo) {
    listaArquivosPasta.innerHTML = `
...
async function renderizarPaginaPdf() {
    ...
    const areaAltura = areaLetraPlayer.clientHeight - 25;
